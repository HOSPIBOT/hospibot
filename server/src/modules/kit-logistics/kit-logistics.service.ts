...
export class KitLogisticsService {
  ...
  async findAll(tenantId: string, query: any) {
    try {
      ...
      const [data, total] = await Promise.all([
        this.prisma.kitLogistics.findMany({ where, orderBy: { createdAt: 'desc' }, skip: (page - 1) * limit, take: Number(limit) }),
        this.prisma.kitLogistics.count({ where }),
      ]);
  ...
  async findOne(tenantId: string, id: string) {
    try { return await this.prisma.kitLogistics.findFirst({ where: { id, tenantId } }); }
    catch (err) { this.logger.error('findOne error', err); return null; }
  }

  async create(tenantId: string, dto: any, userId: string) {
    try {
      return await this.prisma.kitLogistics.create({ data: { ...dto, tenantId, createdBy: userId } });
    } catch (err) { this.logger.error('create error', err); throw err; }
  }

  async update(tenantId: string, id: string, dto: any) {
    try {
      const existing = await this.prisma.kitLogistics.findFirst({ where: { id, tenantId } });
      if (!existing) throw new BadRequestException('Record not found');
      return await this.prisma.kitLogistics.update({ where: { id }, data: dto });
    } catch (err) { this.logger.error('update error', err); throw err; }
  }

  async getStats(tenantId: string) {
    try {
      const total = await this.prisma.kitLogistics.count({ where: { tenantId } });
      return { total };
