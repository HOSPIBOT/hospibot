...
export class DicomViewerService {
  ...
  async findAll(tenantId: string, query: any) {
    try {
      ...
      const [data, total] = await Promise.all([
        this.prisma.dicomStudy.findMany({ where, orderBy: { createdAt: 'desc' }, skip: (page - 1) * limit, take: Number(limit) }),
  ...
  async findOne(tenantId: string, id: string) {
    try { return await this.prisma.dicomStudy.findFirst({ where: { id, tenantId } }); }
    catch (err) { this.logger.error('findOne error', err); return null; }
  }

  async create(tenantId: string, dto: any, userId: string) {
    try {
      return await this.prisma.dicomStudy.create({ data: { ...dto, tenantId, createdBy: userId } });
  ...
  async update(tenantId: string, id: string, dto: any) {
    try {
      ...
      return await this.prisma.dicomStudy.update({ where: { id }, data: dto });
    } catch (err) { this.logger.error('update error', err); throw err; }
  }

  async getStats(tenantId: string) {
    try {
      const total = await this.prisma.dicomStudy.count({ where: { tenantId } });
      return { total };
