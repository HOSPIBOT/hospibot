...
export class OctScansService {
  ...
  async findAll(tenantId: string, query: any) {
    try {
      ...
      const [data, total] = await Promise.all([
        this.prisma.octScan.findMany({ where, orderBy: { createdAt: 'desc' }, skip: (page - 1) * limit, take: Number(limit) }),
  ...
  async create(tenantId: string, dto: any, userId: string) {
    try {
      return await this.prisma.octScan.create({ data: { ...dto, tenantId, createdBy: userId } });
  ...
  async update(tenantId: string, id: string, dto: any) {
    try {
      ...
      return await this.prisma.octScan.update({ where: { id }, data: dto });
    } catch (err) { this.logger.error('update error', err); throw err; }
  }

  async getStats(tenantId: string) {
    try {
      const total = await this.prisma.octScan.count({ where: { tenantId } });
      return { total };
