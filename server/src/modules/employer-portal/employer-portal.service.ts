...
export class EmployerPortalService {
  ...
  async create(tenantId: string, dto: any, userId: string) {
    try {
      return await this.prisma.employerPortal.create({ data: { ...dto, tenantId, createdBy: userId } });
  ...
  async update(tenantId: string, id: string, dto: any) {
    try {
      ...
      return await this.prisma.employerPortal.update({ where: { id }, data: dto });
    } catch (err) { this.logger.error('update error', err); throw err; }
  }

  async getStats(tenantId: string) {
    try {
      const total = await this.prisma.employerPortal.count({ where: { tenantId } });
      return { total };
