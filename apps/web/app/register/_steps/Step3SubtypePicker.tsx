...
export default function Step3SubtypePicker({ groupSlug, groupName, value, onChange }: Props) {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-500 text-center mb-6">{groupName || groupSlug} — {subtypes.length} subtypes available</p>
        ...
        {subtypes.map(s => (
          <div key={s.slug} onClick={() => onChange(s.slug)}
            className={`flex items-center gap-3.5 p-3.5 rounded-xl border cursor-pointer transition-all duration-200
              ...
              ${value === s.slug ? 'border-2 border-[#0D7C66] bg-[#E8F5F0] shadow-sm' : 'border-gray-200 bg-white'}`}>
            <div className={`w-11 h-11 rounded-lg flex items-center justify-center text-lg flex-shrink-0
              ${value === s.slug ? 'bg-[#0D7C66]' : 'bg-[#E8F5F0]'}`}>
              {s.icon || '🏥'}
            </div>
