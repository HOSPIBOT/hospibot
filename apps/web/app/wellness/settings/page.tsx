...
function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean)=>void }) {
  return (
    <button onClick={()=>onChange(!checked)} className={`relative w-10 h-5 rounded-full transition-colors ${checked?'bg-[#BE185D]':'bg-slate-200'}`}>
...
export default function PortalSettingsPage() {
  ...
  const save = async()=>{
    setSaving(true);
    try { await api.patch('/tenants/current/settings',{wellness:cfg}); toast.success('Settings saved!'); }
    catch { toast.error('Failed'); } finally { setSaving(false); }
  };
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900">Wellness Centre Settings</h1>
        <button onClick={save} disabled={saving} className="flex items-center gap-2 text-white text-sm font-semibold px-5 py-2.5 rounded-xl disabled:opacity-60 hover:opacity-90" style={{background:'#BE185D'}}>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[{k:'orgName',l:'Name',p:'Wellness Centre'},{k:'phone',l:'Phone',p:'+91 98765 43210'},{k:'email',l:'Email',p:'info@business.com'}].map(f=>(
