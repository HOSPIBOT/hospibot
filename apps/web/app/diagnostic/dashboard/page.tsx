...
import { FlaskConical, Calendar, MessageSquare, Clock, ArrowUpRight, TrendingUp, Activity } from 'lucide-react';
...
export default function DiagnosticDashboard() {
  ...
  const theme = FALLBACK_THEMES['diagnostic'];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard label=Reports Today value="234" sub="198 delivered" change={"+12%"} icon={FlaskConical} color="#1E3A5F" />
        <KpiCard label=Test Bookings value="89" sub="34 pending" change={"+8"} icon={Calendar} color="#06B6D4" />
        <KpiCard label=WhatsApp Sent value="312" sub="8 pending" change={"+18%"} icon={MessageSquare} color="#25D366" />
        <KpiCard label=Avg TAT value="4.2h" sub="Target: 6h" change={"-22%"} icon={Clock} color="#8B5CF6" />
      </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={revenueData} margin={{ top: 0, right: 0, left: -25, bottom: 0 }}>
              <defs>
                <linearGradient id="diagnosticGrad" x1="0" y1="0" x2="0" y2="1">
              ...
              <Area type="monotone" dataKey="value" stroke={theme.primaryColor} strokeWidth={2.5} fill="url(#diagnosticGrad)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
