      ...
      <div className="grid grid-cols-4 gap-4">
        {[{l:'This Month Revenue',v:'₹3,45,000',i:TrendingUp},{l:'Active Members',v:'48',i:Users},{l:'Sessions This Week',v:'28',i:Calendar},{l:'Satisfaction Score',v:'4.8/5',i:BarChart3}].map(k=>(
          <div key={k.l} className="bg-white rounded-2xl border border-slate-100 p-5">
            <div className="flex items-center gap-2 mb-2"><k.i className="w-4 h-4" style={{color:'#6B21A8'}}/><p className="text-xs text-slate-500">{k.l}</p></div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-5">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={weekRevenue}>
              <defs><linearGradient id="ag" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#6B21A8" stopOpacity={0.15}/><stop offset="95%" stopColor="#6B21A8" stopOpacity={0}/></linearGradient></defs>
              ...
              <Tooltip contentStyle={{borderRadius:10,border:'none',fontSize:12}}/><Area type="monotone" dataKey="revenue" stroke="#6B21A8" strokeWidth={2.5} fill="url(#ag)"/>
            </AreaChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-white rounded-2xl border border-slate-100 p-5">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={monthAppt}>
              ...
              <Tooltip contentStyle={{borderRadius:10,border:'none',fontSize:12}}/><Bar dataKey="sessions" fill="#6B21A8" radius={[4,4,0,0]}/>
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
